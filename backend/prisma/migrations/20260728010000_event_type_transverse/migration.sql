...
-- 1) Dédoublonnage par nom : repointer les events vers l'exemplaire conservé (min id par nom).
UPDATE "events" e
SET "event_type_id" = k."keep_id"
FROM "event_types" et
JOIN (SELECT "name", MIN("id") AS "keep_id" FROM "event_types" GROUP BY "name") k
  ON k."name" = et."name"
WHERE e."event_type_id" = et."id"
  AND e."event_type_id" <> k."keep_id";

-- 2) Supprimer les doublons (on conserve l'exemplaire min id par nom).
DELETE FROM "event_types" et
USING (SELECT "name", MIN("id") AS "keep_id" FROM "event_types" GROUP BY "name") k
