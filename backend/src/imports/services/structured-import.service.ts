...
export class StructuredImportService {
  ...
  async import(content: string, contentType?: string | null): Promise<ImportJobWithAttachment> {
    ...
    try {
      ...
      // Validate — rejet sans correction.
      await this.jobs.transition(job.id, ImportJobStatus.VALIDATING, correlationId);
      const { valid, rejected } = this.validateStage.validate(rawEvents);

      // Normalize — projection vers le modèle commun (noms de référentiels).
      await this.jobs.transition(job.id, ImportJobStatus.NORMALIZING, correlationId);
      const normalized = valid.map((rawEvent) => this.normalizeStage.normalize(rawEvent));

      // Deduplicate — idempotence (providerKey) + signature métier.
      await this.jobs.transition(job.id, ImportJobStatus.DEDUPLICATING, correlationId);
      const knownKeys = await this.pipeline.findKnownProviderKeys(connector.providerId, job.id);
      const { kept, duplicates } = this.dedupeStage.dedupe(normalized, { keys: knownKeys });

      // Persist — EventCandidates (validation humaine) + volumétrie + transition finale.
      await this.jobs.transition(job.id, ImportJobStatus.PERSISTING, correlationId);
